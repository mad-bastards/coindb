 ...
 *
 * - When it is a simple object, its properties are enumerated to
 *   represent both column names and property names within the source
 *   objects. See also option `inherit` that's applicable in this
 *   case.
 *
 * - When it is a single {@link helpers.Column Column} object,
 *   property {@link helpers.ColumnSet#columns columns} is initialized
 *   with just a single column. It is not a unique situation when only
 *   a single column is required for an update operation.
 *
 * - When it is an array, each element is assumed to represent details
 *   for a column. If the element is already of type {@link
 *   helpers.Column Column}, it is used directly; otherwise the
 *   element is passed into {@link helpers.Column Column} constructor
 *   for initialization. On any duplicate column name (case-sensitive)
 *   it will throw {@link external:Error Error} = `Duplicate column
 *   name "name".`
 *
 * - When it is none of the above, it will throw {@link
 *   external:TypeError TypeError} = `Invalid parameter 'columns'
 *   specified.`
 *
 ...
 *
 * It can be used as the default for methods {@link helpers.insert
 * insert} and {@link helpers.update update} when their parameter
 ...
 *
 * By default, only the object's own properties are enumerated for
 * column names.
 *
